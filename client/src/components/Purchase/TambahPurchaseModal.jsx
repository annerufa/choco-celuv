...
export default function TambahPurchaseModal({ isOpen, onClose, onSuccess, itemList }) {
    ...
    function addItem() {
        setFormData(prev => ({
            ...prev,
            items: [...prev.items, { item_id: '', qty: '', unit_price: '' }],
        }));
    }
    ...
    function validate() {
        ...
        formData.items.forEach((item, i) => {
            ...
            if (!item.qty || item.qty <= 0) errs[`qty_${i}`] = 'Qty tidak valid';
            if (!item.unit_price || item.unit_price <= 0) {
                errs[`unit_price_${i}`] = 'Harga tidak valid';
            }
        });
        return errs;
    }
    ...
    const total = formData.items.reduce((sum, item) =>
        sum + (Number(item.qty) * Number(item.unit_price) || 0), 0
    );
                                    ...
                                    {formData.items.map((item, i) => (
                                        <tr key={i}>
                                            <td>
                                                ...
                                                {errors[`item_${i}`] && (
                                                    <span className={styles.errorMsg}>{errors[`item_${i}`]}</span>
                                                )}
                                            </td>
                                            <td>
                                                <input
                                                ...
                                                />
                                                {errors[`qty_${i}`] && (
                                                    <span className={styles.errorMsg}>{errors[`qty_${i}`]}</span>
                                                )}
                                            </td>
                                            <td>
                                                ...
                                                {errors[`unit_price_${i}`] && (
                                                    <span className={styles.errorMsg}>{errors[`unit_price_${i}`]}</span>
                                                )}
                                            </td>
                                            <td className={styles.subtotal}>
                                                Rp {(Number(item.qty) * Number(item.unit_price) || 0).toLocaleString('id')}
                                            </td>
                                            <td>
