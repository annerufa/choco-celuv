...
export default function BoothTable({ boothList, setBoothList, loading, error }) {
    ...
    async function handleTambahBooth(newBooth) {
        ...
        if (!response.ok) throw new Error('Gagal menyimpan booth');
                            ...
                            {filtered.length === 0 ? (
                                ...
                                <tr key={b.id}>
                                    <td>
                                        ...
                                        <div className={styles.tblAddr}>{b.phone || '—'}</div>
                                    </td>
                                    <td className={styles.monoCell}>{b.code}</td>
                                    ...
                                    <td>
                                        <span className={`${styles.badge} ${b.active ? styles.badgeOn : styles.badgeOff}`}>
                    ...
                    <div className={`${styles.drawer} ${styles.show}`}>
                        <div className={styles.drawerHead}>
                            <div
                                ...
                                style={{ background: drawerBooth.active ? 'var(--accent)' : 'var(--brown-300)' }}
                            >
                            ...
                            <div className={styles.drawerInfo}>
                                ...
                                <div className={styles.drawerId}>
                                    ID #{String(drawerBooth.id).padStart(3, '0')} · {drawerBooth.active ? 'Aktif' : 'Nonaktif'}
                                </div>
                            </div>
                        ...
                        <div className={styles.drawerBody}>
                            <div className={styles.detailSection}>
                                ...
                                {[
                                    ...
                                    ['Telepon', drawerBooth.phone || '—'],
                                    ['Kode', drawerBooth.code],
                                    ['Dibuat', drawerBooth.created_at ? new Date(drawerBooth.created_at).toLocaleDateString('id-ID') : '—'],
                                ].map(([key, val]) => (
                            ...
                            <div className={styles.detailSection}>
                                <div className={styles.detailSectionTitle}>Statistik Bulan Ini</div>
                                <div className={styles.detailRow}>
                                    <span className={styles.detailKey}>Pendapatan</span>
                                    <span className={styles.detailVal} style={{ color: 'var(--accent)', fontWeight: 700 }}>
                                        {drawerBooth.rev || '—'}
                                    </span>
                                </div>
                                <div className={styles.detailRow}>
                                    <span className={styles.detailKey}>Transaksi</span>
                                    <span className={styles.detailVal}>
                                        {drawerBooth.trx ? `${drawerBooth.trx} transaksi` : '—'}
                                    </span>
                                </div>
                                <div className={styles.detailRow}>
                                    <span className={styles.detailKey}>Adonan Kadaluarsa</span>
                                    <span className={styles.detailVal} style={{ color: '#b91c1c' }}>
                                        {drawerBooth.expired || '—'}
                                    </span>
                                </div>
                            </div>
